...
const getStravaAuthUrl = createServerFn({
  method: "GET"
}).handler(createSsrRpc("57e57f9ad3a052abb4ff80e36000664d4271aaf52959c15686bf6cd3dd155e55"));
const exchangeStravaCode = createServerFn({
  method: "POST"
}).inputValidator((input) => input).handler(createSsrRpc("fa770a19229d28f852abdf85c9ce2b503e8a91dbf68339ba2dd4cc0f8e9e2688"));
createServerFn({
  method: "POST"
}).inputValidator((input) => input).handler(createSsrRpc("9d83f4c10be9a0051dbe5803dfbe0bf8a9ef204b527c9a88ad31cc0928670431"));
const getAthlete = createServerFn({
  method: "GET"
}).inputValidator((input) => input).handler(createSsrRpc("ea3e6ffcf7f8567fd9ff559f16d3bf6185ca1c045c3183591d4c9704507a53cd"));
const getActivities = createServerFn({
  method: "GET"
}).inputValidator((input) => input).handler(createSsrRpc("8d24b1404747be8b48fd30bdf931924de3d897236fcb5d242f53f38b7ca5789d"));
createServerFn({
  method: "GET"
}).inputValidator((input) => input).handler(createSsrRpc("ceea8d9bfc07bfde12a43c7591ad58bbc0153f62cca4495bf47d9102adc8cc32"));
const getStats = createServerFn({
  method: "GET"
}).inputValidator((input) => input).handler(createSsrRpc("730706da68dc73f668d21e04610a4a25811254b36a37cb962d4a7b09a7e8f8cc"));
const useStravaLogin = () => {
  return useMutation({
    mutationFn: async () => {
...
const useActivities = (page = 1, perPage = 20) => {
  return useQuery({
...
const kmToMiles = (km) => Math.round(km * KM_TO_MILES * 10) / 10;
const metersToFeet = (meters) => Math.round(meters * METERS_TO_FEET);
const mToKm = (m) => m / 1e3;
const aggregate3MonthData = (activities) => {
  ...
  const data = Array.from(monthMap.entries()).map(([month, data2]) => ({
    month,
    distance: kmToMiles(mToKm(data2.distance)),
    activities: data2.activities,
    elevation: Math.round(metersToFeet(data2.elevation))
  })).sort((a, b) => {
...
const calculate3MonthRunStats = (activities) => {
  const now = /* @__PURE__ */ new Date();
  const threeMonthsAgo = new Date(now.getFullYear(), now.getMonth() - 3, now.getDate());
  const runs = activities.filter(
    (a) => (a.type === "Run" || a.sport_type === "Run") && new Date(a.start_date) >= threeMonthsAgo
  );
  if (runs.length === 0) {
...
const calculateActivityStats = (activities) => {
  const totalDistanceKm = activities.reduce((sum, a) => sum + a.distance, 0) / 1e3;
...
export {
  ...
  useStats as b,
  useLogout as c,
  aggregate3MonthData as d,
  calculateActivityStats as e,
  calculate3MonthRunStats as f,
  useStravaLogin as g,
  useStravaCallback as h,
  kmToMiles as k,
  metersToFeet as m,
  useAthlete as u
};
